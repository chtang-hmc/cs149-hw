...
void clampedExpVector(float *values, int *exponents, float *output, int N)
{
  ...
  for (int i = 0; i < N; i += VECTOR_WIDTH)
  {
    ...
    // set y == 0 mask
    _cs149_veq_int(maskYZero, y, zeroInt, maskAll); // if (y == 0) {

    // execute if
    _cs149_vmove_float(result, oneFloat, maskYZero); // output[i] = 1.f;

    // inverse mask to generate else
    maskYNotZero = _cs149_mask_not(maskYZero); // } else {

    // execute else
    _cs149_vmove_float(result, x, maskYNotZero);     // result = x;
    _cs149_vsub_int(count, y, oneInt, maskYNotZero); // count = y - 1;

    // execute while loop
    _cs149_vgt_int(maskCountNotZero, count, zeroInt, maskYNotZero); // set conditional
    while (_cs149_cntbits(maskCountNotZero) > 0)
    {
      _cs149_vmult_float(result, result, x, maskCountNotZero);        // result *= x;
      _cs149_vsub_int(count, count, oneInt, maskCountNotZero);        // count--;
      _cs149_vgt_int(maskCountNotZero, count, zeroInt, maskYNotZero); // update conditional
    }

    // clamp result
    _cs149_vgt_float(maskOverMax, result, max, maskYNotZero); // if (result > 9.999999f) {
    _cs149_vmove_float(result, max, maskOverMax);             // result = 9.999999f; }
...
float arraySumVector(float *values, int N)
{
  __cs149_vec_float x[N / VECTOR_WIDTH];
  ...
  {
    maskAll = _cs149_init_ones();
    _cs149_vload_float(x[i], values + i * VECTOR_WIDTH, maskAll); // x = values[i];
    _cs149_vadd_float(sum, sum, x[i], maskAll);                   // sum += x;
  }

  // Store sum in an array
  float *sumArray = new float[VECTOR_WIDTH];
